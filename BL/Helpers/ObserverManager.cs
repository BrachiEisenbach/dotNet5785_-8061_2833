...
class ObserverManager //stage 5
{
    ...
    {
        if (_listObservers == null) return;

        foreach (var handler in _listObservers.GetInvocationList())
        {
            try
            {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"שגיאה בצופה: {ex.GetType().Name} - {ex.Message}");
